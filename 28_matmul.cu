...
/*
 * Method Shared Memory Blocking (26.7-28.1% speed of cuBLAS.)
 * Computes: C = A * B where A is (M x K) and B is (K x N)
 * Modified from source: https://siboehm.com/articles/22/CUDA-MMM (Kernel #3)
 */

__global__ void customMatMul(float* A, float* B, float* C, int M, int K, int N) {
    // Simon's approach
    //      26.7-28.1% speed of cuBLAS on my A100
    // Using a x (col), y (row) grid of threads
    //      26.7-28.1% speed of cuBLAS on my A100
    // the output block that we want to compute in this threadblock
    // IOW: we can use the more idiomatic 2D thread-block structure
    ...
    for (int bkIdx = 0; bkIdx < K; bkIdx += BLOCK_SIZE){
        // Each thread loads one of the elements in A & B
        ...
        // execute the partial dotproduct on the currently cacehd block
