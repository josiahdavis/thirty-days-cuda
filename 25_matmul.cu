 ...
 * Compile: nvcc 25_matmul.cu -o main -lcublas
 * Run: ./main
 */
...
#define TILE_WIDTH 4 // each thread calculates 4 elements along the row
...
/*
 * Method 1: Shared Memory Matmul with 1D blocktiling (31.6% speed of cuBLAS.)
...
__global__ void customMatMul(float* A, float* B, float* C, int M, int K, int N) {
    ...
    __shared__ float Bs[BLOCK_SIZE][BLOCK_SIZE * TILE_WIDTH];

    int base_idx = (blockDim.x * blockIdx.x + threadIdx.x) * TILE_WIDTH;
    int idy = blockDim.y * blockIdx.y + threadIdx.y;

    float temp[TILE_WIDTH] = {0.0f}; // array to hold multiple partials
    int numBlocks = (K + BLOCK_SIZE - 1) / BLOCK_SIZE;

    for (int bk = 0; bk < numBlocks; bk++){

        // 1. Outer loop: load block into shared memory
        int aCol = bk * BLOCK_SIZE + threadIdx.x;
        if (idy < M && aCol < K){
            As[threadIdx.y][threadIdx.x] = A[idy * K + aCol];
        } else {
            As[threadIdx.y][threadIdx.x] = 0.0f;
        }
        int bRow = bk * BLOCK_SIZE + threadIdx.y;
        for (int t = 0; t < TILE_WIDTH; t++){
            int bCol = base_idx + t;
            if (bRow < K && bCol < N){
                Bs[threadIdx.y][threadIdx.x * TILE_WIDTH + t] = B[bRow * N + bCol];
            } else {
                Bs[threadIdx.y][threadIdx.x * TILE_WIDTH + t] = 0.0f;
            }
        }
        __syncthreads();
        
        // 2. Inner loop: calculate partial dot product on the cached block.
        for (int k = 0; k < BLOCK_SIZE; k++){
            float a_val = As[threadIdx.y][k];
            for (int t = 0; t < TILE_WIDTH; t++){
                temp[t] += a_val * Bs[k][threadIdx.x * TILE_WIDTH + t];
            }
        }
        __syncthreads();
    }

    // 3. Write to output after iterating through all blocks
    for (int t = 0; t < TILE_WIDTH; t++){
        int col_idx = base_idx + t;
        if (idy < M && col_idx < N){
            C[idy * N + col_idx] = temp[t];
        }
    }
}
...
int main() {
    ...
    dim3 gridDim((N + BLOCK_SIZE * TILE_WIDTH - 1) / (BLOCK_SIZE * TILE_WIDTH), (M + BLOCK_SIZE - 1) / BLOCK_SIZE);
