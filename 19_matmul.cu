/*
 * Comparison of custom Matrix Multiplication Global Memory Coalescing with CUBLAS.
 * Compile: nvcc 19_matmul.cu -o main -lcublas
 * Run: ./main
 */
...
/*
 * Method 1: custom Matrix Multiplication Kernel using shared memory
...
__global__ void customMatMul(float* A, float* B, float* C, int M, int K, int N) {
    ...
    for (int bkIdx = 0; bkIdx < K; bkIdx += BLOCK_SIZE){
        ...
        // partial dot product on currently cached block
