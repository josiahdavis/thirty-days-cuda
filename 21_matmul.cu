 ...
 * Compile: nvcc 21_matmul.cu -o main -lcublas
 * Run: ./main
 */
...
/*
 * Method 1: Blocktiling for Calculating Multiple Results per Thread (56% speed of cuBLAS.)
 * Computes: C = A * B where A is (M x K) and B is (K x N)
 * Source: https://siboehm.com/articles/22/CUDA-MMM (Kernel 4)
 */
...
__global__ void __launch_bounds__((BM * BN) / (TM * TN), 1) customMatMul(float* A, float* B, float* C, int M, int K, int N) {
    const uint cRow = blockIdx.y;
    const uint cCol = blockIdx.x;

    const uint totalResultsBlockTile = BM * BN;
    // each thread is tasked with calculating TN*TN elements in blocktile
    ...
    // calculating the indices this thread will load into shared
    const uint innerRowA = threadIdx.x / BK;
    const uint innerColA = threadIdx.x % BK;
    // calculates the number of rows of As that are being loaded into a single step
    ...
    // outer-most loop over block iles
